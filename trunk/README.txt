...
This plugin enables Microsoft Team Foundation Server revision control in FitNesse.
The plugin is intended to perform basic operations like opening pages for edit,
delete, add and revert files if neccessary. Remember that each page is defined
by a directory that bears it's name, and two files that contain it's content.
The first file is content.txt which holds the wiki text. The second is 
...
people who would like to include these files in the version control, too.
